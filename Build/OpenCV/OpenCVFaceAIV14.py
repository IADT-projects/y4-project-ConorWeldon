...
def detect_anger(face_roi_gray, face_roi_color):
    ...
    for (fx, fy, fw, fh) in faces:
        ...
        # the mouth is in a "down" orientation
        eyebrow_detected = False
        for (ex, ey, ew, eh) in eyes:
            for (exb, eyb, ewb, ehb) in eye_cascade.detectMultiScale(roi_gray, scaleFactor=1.1, minNeighbors=5):
                if ex == exb and ey == eyb and ew == ewb and eh == ehb:
        ...
        mouth_down_distance = 0  # set a default value
        ...
        if eyebrow_detected and mouth_detected:
            anger_detected = True

    return anger_detected, mouth_detected
...
def recognize_emotion_and_face():
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            anger_detected, mouth_detected = detect_anger(face_roi_gray, face_roi_color)
            ...
            if len(eyes) > 1 and len(smiles) > 0:
                ...
                for (sx, sy, sw, sh) in smiles:
                    ...
                    if confidence_percent > 80:
                        label = 'Happy'
                    elif confidence_percent < -150 and confidence_percent > -250:
                        label = 'Anger'
                    elif confidence_percent < -250:
                        label = 'Sad'
                    else:
                        label = 'Neutral'
